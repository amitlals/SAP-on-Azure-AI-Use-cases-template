<!-- SAP, Microsoft, and Copilot Logos -->
<p align="center">
  <img src="https://upload.wikimedia.org/wikipedia/commons/5/59/SAP_2011_logo.svg" alt="SAP Logo" height="60">
  &nbsp;
  <img src="https://upload.wikimedia.org/wikipedia/commons/4/44/Microsoft_logo.svg" alt="Microsoft Logo" height="60">
  &nbsp;
  <img src="https://raw.githubusercontent.com/gilbarbara/logos/master/logos/github-copilot.svg" alt="GitHub Copilot" height="60">
</p>

<h1 align="center">SAP on Microsoft Platform & SAP + Azure AI Use Cases</h1>
<p align="center">
  <b>Unlocking Innovation with SAP, Azure AI, GitHub Copilot, Joule Copilot</b>
  <br>
  <i>Templates for practical business scenarios, integration blueprints, and automation with J4D, GHCP</i>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/SAP-Microsoft-blue?logo=sap&logoColor=white">
  <img src="https://img.shields.io/badge/Azure%20AI-Enabled-blueviolet?logo=microsoftazure">
  <img src="https://img.shields.io/badge/GitHub%20Copilot-AI%20Assistant-4B8BBE?logo=githubcopilot">
  <img src="https://img.shields.io/github/license/amitlals/SAP-on-Azure-AI-Use-cases-template">
  <img src="https://img.shields.io/github/languages/top/amitlals/SAP-on-Azure-AI-Use-cases-template">
  <img src="https://img.shields.io/github/last-commit/amitlals/SAP-on-Azure-AI-Use-cases-template">
</p>

---

## 🌟 Overview

This template repository delivers ready-to-use use cases, guides, and automation for integrating SAP business applications with Microsoft Azure AI. Designed for developers, solution architects, and business innovators, it accelerates digital transformation with real-world use cases. Running J4D, GHCP, ADT platforms

## 📚 Quick Navigation

- [🚀 Getting Started](GETTING_STARTED.md) - Setup guide and prerequisites
- [📁 Use Cases](use-cases/) - Detailed documentation for each scenario
- [🤝 Contributing](CONTRIBUTING.md) - How to contribute to this repository
- [🔒 Security](SECURITY.md) - Security guidelines and best practices
- [👥 Code of Conduct](CODE_OF_CONDUCT.md) - Community guidelines

---

## 🎯 Featured Use Cases

Below are 8 powerful SAP + Azure AI integration scenarios. Click on each to explore detailed documentation and setup guides.

### 1. 📱 Screenshot to Fiori App Frontend Design (Under 5 minutes)
Transform UI screenshots into functional Fiori applications using AI-powered development tools.

**[📖 View Detailed Documentation](use-cases/01-screenshot-to-fiori/)**

![final](https://github.com/user-attachments/assets/a24e7736-0aa6-461f-8b3b-bb01a55ee14c)

**Key Technologies**: SAP Fiori, GitHub Copilot, UI5 | **Difficulty**: Intermediate | **Time**: 5-10 min

<br> <hr>


### 2. 🤖 Power of Two Copilots - SAP J4D and GitHub Copilot
Leverage both SAP Joule for Developers and GitHub Copilot for enhanced development productivity.

**[📖 View Detailed Documentation](use-cases/02-dual-copilot/)**

![new](https://github.com/user-attachments/assets/5a5472f9-5baa-4816-8a51-4be8431c87d4)

**Key Technologies**: J4D, GitHub Copilot, SAP BAS | **Difficulty**: Beginner-Intermediate | **Time**: 15-20 min

<br> <HR>
### 3. 🎨 Fiori App Development with Joule
Build Fiori applications using SAP Joule's AI-powered assistance for code generation.

**[📖 View Detailed Documentation](use-cases/03-fiori-with-joule/)**

![FIORI1](https://github.com/user-attachments/assets/6a07aab5-e98f-4313-9970-526439e7512b)

**Key Technologies**: SAP Fiori, Joule, SAPUI5 | **Difficulty**: Intermediate | **Time**: 20-30 min
*(Source: SAP)*

<br><hr>
### 4. 🛠️ SAP Build Low-Code Solutions with Joule Copilot
Create enterprise applications using SAP Build's low-code platform with AI assistance.

**[📖 View Detailed Documentation](use-cases/04-sap-build-lowcode/)**

<i>⚡ Please watch in full-screen mode for best experience!</i>

https://github.com/user-attachments/assets/eb293552-37c6-454f-bd51-23deab22b4e2

https://github.com/user-attachments/assets/0b4379b1-5be6-4e9c-adcc-2a80b2a4f164

**Key Technologies**: SAP Build, Joule Copilot, Low-Code | **Difficulty**: Beginner | **Time**: 15-25 min

<br><hr>
### 5. 🔧 Azure SRE Agent Integration - Agentic DevOps
Implement agentic DevOps with Azure SRE Agent for SAP workloads running on Azure.

**[📖 View Detailed Documentation](use-cases/05-azure-sre-agent/)**

🎉 **Now in Public Preview** - No sign-up required! Learn how to integrate with your SAP workloads on Azure.

https://github.com/user-attachments/assets/2fc4ae78-7c28-43bc-8075-b48331b2c660

**Key Technologies**: Azure SRE Agent, SAP on Azure, DevOps | **Difficulty**: Advanced | **Time**: 30-45 min



<br><hr>
### 6. 💬 SAP Joule AMA (Ask Me Anything)
Explore SAP Joule's conversational AI capabilities for querying SAP systems and data.

**[📖 View Detailed Documentation](use-cases/06-joule-ama/)**

SAP Joule is becoming a powerful tool for SAP customers. Watch this quick demo of the AMA capabilities:

https://github.com/user-attachments/assets/2c73eb84-2460-417d-8dba-fbe7902accab

**Key Technologies**: SAP Joule, Natural Language Processing | **Difficulty**: Beginner | **Time**: 10-15 min

<br><hr>
### 7. 📊 SAP-RPT-1-OSS Tabular LLM Playground
Work with SAP's open-source tabular LLM for structured data processing and analysis.

**[📖 View Detailed Documentation](use-cases/07-sap-rpt-oss/)**

Hands-on with [SAP-RPT-1-OSS](https://github.com/sap-samples/sap-rpt-1-oss/) - SAP's open-source tabular LLM:

<img width="839" height="540" alt="image" src="https://github.com/user-attachments/assets/4ef2737c-ebee-4d49-a17b-e86ce9652cca" />

**Key Technologies**: SAP RPT-1-OSS, Python, ML | **Difficulty**: Advanced | **Time**: 45-60 min



<br><hr>
### 8. 🔐 Foundry Local - On-Device AI with Privacy
Run AI models locally on your device with complete data privacy - no Azure subscription required.

**[📖 View Detailed Documentation](use-cases/08-foundry-local/)**

Hands-on with [Foundry Local](https://www.foundrylocal.ai/) - on-device inference with complete data privacy:

![Foundry-Local-GIF3](https://github.com/user-attachments/assets/3a63e1a5-151b-46fd-a10e-87b8c28afc2b)

**Key Technologies**: Foundry Local, On-Device AI, Privacy-First | **Difficulty**: Intermediate | **Time**: 20-30 min




---

## 🚀 Getting Started

Ready to get started? Follow these steps:

1. **📖 Read the [Getting Started Guide](GETTING_STARTED.md)** - Setup prerequisites and environment
2. **🎯 Choose a Use Case** - Pick a scenario that matches your needs
3. **📋 Follow the Documentation** - Each use case has detailed setup instructions
4. **💻 Start Building** - Clone the templates and adapt to your requirements
5. **🤝 Contribute Back** - Share your learnings and improvements

### Quick Links
- [📁 Browse All Use Cases](use-cases/)
- [🔧 Prerequisites & Setup](GETTING_STARTED.md)
- [🤝 Contribution Guidelines](CONTRIBUTING.md)
- [🔒 Security Best Practices](SECURITY.md)

## ⚙️ Usage Guide
- These examples are for demo and learning purposes
- Explore the scenarios, adapt the templates, and validate in your own SAP on Azure environment before production
- Sign up for [Joule for Developers program](https://developers.sap.com/joule)
- Ensure you have required access to SAP BTP and Azure subscriptions

---

## 📚 Resources & Documentation

### SAP Resources
- [SAP on Azure Overview](https://learn.microsoft.com/en-us/azure/sap)
- [SAP Business Application Studio](https://help.sap.com/docs/SAP%20Business%20Application%20Studio)
- [SAP Joule Documentation](https://help.sap.com/docs/joule)
- [SAP Build](https://help.sap.com/docs/build)
- [SAP Fiori Design Guidelines](https://experience.sap.com/fiori-design/)

### Azure Resources
- [Azure AI Services](https://learn.microsoft.com/en-us/azure/ai-services/)
- [Azure OpenAI Service](https://learn.microsoft.com/en-us/azure/ai-services/openai/)
- [Azure Cognitive Services](https://learn.microsoft.com/en-us/azure/cognitive-services/)
- [Azure SRE Agent](https://learn.microsoft.com/en-us/azure/chaos-studio/)

### Development Tools
- [GitHub Copilot](https://github.com/features/copilot)
- [GitHub Actions](https://github.com/features/actions)
- [VS Code SAP Extensions](https://marketplace.visualstudio.com/search?term=SAP&target=VSCode)

### Community & Support
- [SAP Community](https://community.sap.com/)
- [Microsoft Tech Community](https://techcommunity.microsoft.com/)
- [GitHub Discussions](https://github.com/amitlals/SAP-on-Azure-AI-Use-cases-template/discussions)

---

## 🤝 Contributing

We welcome contributions from the community! Whether it's:
- 🐛 Reporting bugs
- 💡 Suggesting new use cases
- 📝 Improving documentation
- 🔧 Submitting code improvements

Please read our [Contributing Guidelines](CONTRIBUTING.md) to get started.

## 📄 License

This project is licensed under the MIT License - see the [LICENSE](LICENSE) file for details.

---
## ⚠️ Disclaimer⚠️ 
Content here is for demo and educational purposes only — not professional, legal, or commercial advice.
Always validate suitability, security, licensing, and compliance with your organization’s policies and third-party terms before using in production.

Do not use real customer data, credentials, or sensitive info when testing.
SAP, Microsoft, GitHub, and other names/logos are trademarks of their respective owners.
This content is shared in a personal capacity, independent of my employment.

For official deployment or support, please engage the appropriate vendor or certified partner.
---

<p align="center">
  <b>🌐 Empower your SAP solutions with Microsoft Azure AI and Copilot!</b>
</p>
